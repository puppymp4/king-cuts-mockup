...
const jobs = [
  ...
  // Reviewer avatars (Booksy public profile photos)
  { in: "source-images/avatar-jesus.jpeg", out: "images/avatar-jesus.webp", w: 200, q: 82, square: true },
  { in: "source-images/avatar-diego.jpeg", out: "images/avatar-diego.webp", w: 200, q: 82, square: true },
...
for (const j of jobs) {
  ...
  const pipeline = j.square
    ? sharp(j.in).resize({ width: j.w, height: j.w, fit: "cover", position: "centre", withoutEnlargement: true })
    : sharp(j.in).resize({ width: j.w, withoutEnlargement: true });
  await pipeline.webp({ quality: j.q }).toFile(j.out);
